...
function writeSvgThumbnail(category, variantId, title, subtitle) {
  ...
  <text x="50%" y="45%" font-family="system-ui, sans-serif" font-weight="600" font-size="14" fill="#1e293b" text-anchor="middle">${title.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</text>
...
const definitions = {
  ...
  tags: Array.from({length: 10}, (_, i) => ({
    ...
    styles: { display: 'flex', flexWrap: 'wrap', gap: '12px', padding: '48px', justifyContent: 'center' },
    dynamic: { category: 'tags', source: 'routes', limit: 10 + i, order: 'count.posts desc', layoutStyle: 'style-' + i }
  })),
  products: Array.from({length: 10}, (_, i) => ({
    name: 'Product Layout ' + (i+1),
    styles: { display: 'grid', gridTemplateColumns: 'repeat(' + ((i%4)+1) + ', 1fr)', gap: '32px', padding: '64px' }
  })),
  recommendations: Array.from({length: 10}, (_, i) => ({
    name: 'Recommendation Grid ' + (i+1),
    styles: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', padding: '48px' },
    dynamic: { category: 'recommendations', source: 'related', limit: 3 + (i%3), order: 'published_at desc', layoutStyle: 'grid' }
  })),
  faq: Array.from({length: 10}, (_, i) => ({
    name: 'FAQ Style ' + (i+1),
    styles: { display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '800px', margin: '0 auto', padding: '64px' }
  }))
};
...
Object.entries(definitions).forEach(([category, variants]) => {
  ...
  variants.forEach((variant, index) => {
    ...
    const thumbnailUri = writeSvgThumbnail(category, presetId, variantName.toUpperCase(), category.toUpperCase());
    ...
    if (variant.props) {
      ...
      Object.entries(variant.props).forEach(([k, v]) => {
        if (v === 'navigation') {
          ...
          const label = k.includes('signin') ? 'Sign In' : 'Subscribe';
...
console.log('Successfully generated v5 with JSON stringify, SVG files, and strict bindings.');
